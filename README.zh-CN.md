...
上面的弱模型探测把那个问题真正有信息量的版本推迟了：不是"弱模型自己的规则能不能帮到自己"（被蒸馏器崩溃的 bug 污染了），而是**一条从强模型（Haiku）已解决的 episode 中蒸馏出的规则，能不能帮到弱模型（gpt-4o-mini）解决同一错误类别下一个不同的、未见过的实例？** 这是 v0.2 基准测试补救系列中的最后一个实验。

`getProvider()`（`src/providers/index.ts`）是一个模块级单例，在整个进程生命周期内由 `LLM_PROVIDER` 固定 —— 一个进程不能同时跑一个 Haiku 的种子 solver 和一个 gpt-4o-mini 的迁移 solver。所以这次不是一次配对跑，而是两个阶段，通过一个基于文件的规则库桥接：

```bash
# Phase S（纯 Anthropic 进程）：用强模型解决 3 个指定的种子 fixture，
# 通过现有的证据路径从每个中蒸馏一条规则，写成按场景类型分类的 JSON。
bun run test/demo/live-ab.ts --emit-seed-rules test/demo/seed-rules/

# Phase T（OpenAI 进程）：跳过逐对蒸馏，为该类型加载预先播种好的规则，
# 在真正计算出的匹配等级上注入（不强制为 'fine'）。
...
**在 Phase T 之前就已预先登记**（写入每份结果文件的表头，此后未改动过）：主要终点 = ParseError 成功率（精确 McNemar 检验）；次要终点 = 轮次/token 差异，以及 ExpectMismatch；NullAccess 作为负面对照（预期为零结果 —— 如果这里出现较大变化，说明是测试框架的伪影，而非真实迁移）。主要终点上的显著提升，预先措辞为"证明了跨模型迁移"；零结果则预先措辞为"注入迁移的是知识，而非技能 —— 能力天花板依然存在"。

```
=== ParseError（主要终点，n=20）===
| 指标            | 控制组         | 实验组         | 差异           | p 值    |
|-----------------|---------------|---------------|--------------|---------|
| 轮次            | 4.3 ± 1.3     | 2.0 ± 0.0     | -40% ± 50%    | 0.000   |
| 成功率          | 2/20          | 20/20         | —             | 0.000   |

=== ExpectMismatch（次要终点，n=20）===
| 指标            | 控制组         | 实验组         | 差异           | p 值    |
|-----------------|---------------|---------------|--------------|---------|
| 轮次            | 3.4 ± 1.6     | 3.3 ± 1.8     | +36% ± 137%   | 0.921   |
| 成功率          | 11/20         | 8/20          | —             | 0.453   |

=== NullAccess（负面对照，n=10）===
| 指标            | 控制组         | 实验组         | 差异           | p 值    |
|-----------------|---------------|---------------|--------------|---------|
| 轮次            | 1.0 ± 0.0     | 1.0 ± 0.0     | +0% ± 0%      | 1.000   |
| 成功率          | 10/10         | 10/10         | —             | 1.000   |
```

**在主要终点上，跨模型迁移得到了干净的证明。** `gpt-4o-mini` 单独解决 ParseError 只有 2/20（10%）；而有了 Haiku 从一个*不同*的 ParseError 实例中蒸馏出的规则，它能解到 20/20，而且每一次都恰好是 2 轮（零方差），轮次和成功率上的 p 值都小于 0.001（测试框架打印为 0.000）。逐轮日志显示，这个统一的 2 轮模式是"编辑→编辑"—— 第一次编辑还不能让测试通过，第二次完成修复 —— 实验组里零次读取动作，而且覆盖了轮换中全部五个源文件（也就是说，这条规则在 coarse 匹配等级下也成功迁移了，不仅仅是对种子的同文件"兄弟" fixture 有效）；两条臂跑的是完全相同的协议。负面对照如预先登记的那样精确地打了平手（轮次和成功率都完全打平，p=1.000）—— 说明没有测试框架的伪影在放大主要结果。

ExpectMismatch 这个次要终点是一次打平（轮次 p=0.921，成功率 p=0.453，实验组数字甚至略差一点）—— 这不是矛盾，而是一个值得理解的真实边界条件。种子规则（见下文）在诊断上被限定在*数值*偏移类的算术 bug 上（`a + b - 1`，本意是 `a + b`）；而 ExpectMismatch 的其他实例包含字符串格式化类的不匹配（货币、大小写、补位），一条数值偏移形状的规则对这些完全无话可说，甚至可能造成干扰。相比之下，ParseError 的种子规则诊断的是一个结构性模式（catch 块缺少闭合花括号），足够通用，能覆盖该类型的大多数实例。**这里的结论不是"ParseError 能迁移、ExpectMismatch 不能"—— 而是迁移质量取决于这一个种子实例对目标类型内部真实多样性的代表程度**，这正是重复门槛（要求 ≥2 个种子 episode 才信任一条规则，仍推迟到 v1.0）能够帮上忙的地方。

表头那一行（`Match level: medium`，而非预期的 `coarse`）揭示了一个机制上的细节：`ParseError_A2`/`ParseError_A3` 恰好和种子 fixture `ParseError_A1` 共用同一个文件（`src/parser.ts`），所以它们在 *medium* 等级上匹配（文件也一致），而不是 coarse；其他文件里的实例则在 coarse 等级匹配。匹配等级是通过 `findMatchingRules()` 对每一对真正计算出来的，从未被强制指定 —— 这是 fixture 集合里真实存在的异质性，不是 bug，也不影响解读（medium 依然不是 fine —— 种子的任何实例级细节都没有延续下来，延续下来的只有诊断模式本身）。

种子规则的建议文本（Phase T 之前全部三条都已审阅，均未需要重新播种）：`test/demo/seed-rules/ParseError.json`、`ExpectMismatch.json`、`NullAccess.json`。完整的预注册记录见 `test/demo/results/2026-07-exp2-parseerror-n20.txt`、`2026-07-exp2-expectmismatch-n20.txt`、`2026-07-exp2-nullaccess-n10.txt`。（审计说明：Phase T 的记录表头会打印一个 `Distill model:` 字段 —— 那是逐对模式下的默认值，在播种模式下并未被使用，因为播种模式完全不做蒸馏（`distillation failures: 0`）；真正的规则来源记录在种子 JSON 的 `model_id` 字段里：`claude-haiku-4-5-20251001`。）

这是 v0.2 基准测试补救系列的最后一个实验。接下来的工作是 v1.0 Batch 0（Phase 21+）。
